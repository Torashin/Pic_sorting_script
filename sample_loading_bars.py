...
def demo_one_line_progress_meter():
    ...
    for i in range(10000):
        if not sg.one_line_progress_meter('My 1-line progress meter', i+1, 10000,
                                          'meter key',
                                          'MY MESSAGE1',
                                          'MY MESSAGE 2',
                                          orientation='h',
...
def manually_updated_meter_test():
    ...
    sleep(2)  # more of your code.... perhaps pages and pages of code.
...
def custom_meter_example():
    ...
    # done with loop... n e ed to destroy the window as it's still open
    window.close()
...
def progress_bar(key, iterable, *args, title='', **kwargs):
    ...
    for i, val in enumerate(iterable):
        yield val
        if not sg.one_line_progress_meter(title, i+1, len(iterable), key, *args, **kwa r gs):
            break
